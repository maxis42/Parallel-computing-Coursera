...
RGBApixel* BMP::GetLinePixel(int i)
{
	using namespace std;
	bool Warn = false;
	if (i >= Width)
	...
	if (Warn && EasyBMPwarnings)
	{
		cout << "EasyBMP Warning: Attempted to access non-existent row;" << endl
			<< "                 row in the range [0,"
			<< Width - 1 << "] " << endl;
	}
	return Pixels[i];
...
RGBApixelArr BMP::GetColorsArr(void)
{

	return ColorsStructOfArray;
}

RGBApixel* BMP::GetColors(void)
{
...
bool BMP::SetBitDepth( int NewDepth )
{
 ...
 if( Colors )
 { delete [] Colors; }
 int NumOfCol = IntPow( 2, BitDepth );
 if( BitDepth == 1 || BitDepth == 4 || BitDepth == 8 )
 { 
	 Colors = new RGBApixel [NumOfCol]; 
	 NumberOfColors = NumOfCol;
 }
 else
 { Colors = NULL; } 
 if( BitDepth == 1 || BitDepth == 4 || BitDepth == 8 )
 { 
	 ...
	 if (ColorsStructOfArray.Alpha)
	 {
		 _mm_free(ColorsStructOfArray.Alpha);
	 }
	 if (ColorsStructOfArray.Blue)
	 {
		 _mm_free(ColorsStructOfArray.Blue);
	 }
	 if (ColorsStructOfArray.Green)
	 {
		 _mm_free(ColorsStructOfArray.Green);
	 }
	 if (ColorsStructOfArray.Red)
	 {
		 _mm_free(ColorsStructOfArray.Red);
	 }
	 //ColorsStructOfArray.Alpha = (int*) _mm_malloc(sizeof(int)*NumberOfColors, sizeof(int)*8);
	 //ColorsStructOfArray.Blue = (int*)_mm_malloc(sizeof(int)*NumberOfColors, sizeof(int) * 8);
	 //ColorsStructOfArray.Green = (int*)_mm_malloc(sizeof(int)*NumberOfColors, sizeof(int) * 8);
	 //ColorsStructOfArray.Red = (int*)_mm_malloc(sizeof(int)*NumberOfColors, sizeof(int) * 8);
	 ColorsStructOfArray.Alpha = (int*) _mm_malloc(sizeof(int)*NumberOfColors, 64);
	 ColorsStructOfArray.Blue = (int*)_mm_malloc(sizeof(int)*NumberOfColors, 64);
	 ColorsStructOfArray.Green = (int*)_mm_malloc(sizeof(int)*NumberOfColors, 64);
	 ColorsStructOfArray.Red = (int*)_mm_malloc(sizeof(int)*NumberOfColors, 64);
	 for (int i = 0; i < NumberOfColors; i++)
	 {
		 ColorsStructOfArray.Alpha[i] = Colors[i].Alpha;
		 ColorsStructOfArray.Blue[i] = Colors[i].Blue;
		 ColorsStructOfArray.Green[i] = Colors[i].Green;
		 ColorsStructOfArray.Red[i] = Colors[i].Red;
	 }
 }
...
const char* BMP::GetFileNameImage()
{
...
{ 
 ...
 FileNameImage = new char[strlen(FileName)];
 strcpy(FileNameImage, FileName);
...
// Функция выполняющая Гауссово размытие.
// Входной параметр радиус размытия ( в пикселях)
int BMP::gaussian_blur(int r)
{
	RGBApixel val;
	RGBApixel** tmp_image; // Временное изображение
	tmp_image = new RGBApixel*[Width];
	for (int i = 0; i < Width; i++)
		tmp_image[i] = new RGBApixel[Height];
	float alpha, red, blue, green; // временные переменные хранящие интенсивность каждого канала изображения
	float dsq, wght;
	int x, y;
	// Обход всех пикселей изобраэения
	for (int i = 0; i<Width; i++)
		for (int j = 0; j< Height; j++)
		{
			alpha = 0;
			...
			green = 0;
			float wsum = 0;
			// Обход пикселей в радиусе r от обрабатываемого пикселя
			for (int ix = i - r; ix<i + r + 1; ix++)
				for (int iy = j - r; iy < j + r + 1; iy++)
				{
					x = std::min(Width - 1, std::max(0, ix));
					y = std::min(Height - 1, std::max(0, iy));
					dsq = (ix - i)*(ix - i) + (iy - j)*(iy - j);
					wght = exp(-dsq / (2 * r*r)) / (3.1415 * 2 * r*r); // расчет веса для соотвествующего пикселя окрестности
					alpha += GetPixel(x, y).Alpha * wght;
					blue += GetPixel(x, y).Blue * wght;
					red += GetPixel(x, y).Red * wght;
					green += GetPixel(x, y).Green * wght;
					wsum += wght;
				}
			// нормализация и округление интенсивности для соотвествующего канала
	...
	RGBApixel** tmp = new RGBApixel*[Width];
	for (int i = 0; i < Width; i++)
		tmp[i] = new RGBApixel[Height];
